...
class GeoQuery:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        jgi_count = self.collection.count_documents({"dataset_id": {"$regex": "^jgi:"}})
                ...
                [
                    {
                        "$group": {
                            ...
                            "minLat": {
                                "$min": {
                                    "$arrayElemAt": ["$coordinates.coordinates", 1]
                                }
                            },
                            "maxLat": {
                                "$max": {
                            ...
                            "minLng": {
                                "$min": {
                                    "$arrayElemAt": ["$coordinates.coordinates", 0]
                                }
                            },
                            "maxLng": {
                                "$max": {
    ...
    def find_by_system(
        self, system_name: str, limit: int = 1000
    ) -> List[Dict[str, Any]]:
    ...
    def find_in_box(
        self, west: float, south: float, east: float, north: float, limit: int = 1000
    ) -> List[Dict[str, Any]]:
    ...
    def find_nearby(
        self, lat: float, lng: float, distance: int = 10000, limit: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    def create_map(
        self, points: List[Dict[str, Any]], output_file: str = "geo_map.html"
    ) -> None:
        ...
        lats = [
            p["coordinates"]["coordinates"][1] for p in points if "coordinates" in p
        ]
        lngs = [
            p["coordinates"]["coordinates"][0] for p in points if "coordinates" in p
        ]
    ...
    def export_to_csv(
        self, points: List[Dict[str, Any]], output_file: str = "geo_data.csv"
    ) -> None:
...
def main():
    ...
    parser.add_argument(
        "--system-name", type=str, help="System name for system queries"
    )
    parser.add_argument("--dataset-id", type=str, help="Dataset ID for dataset queries")
    parser.add_argument("--lat", type=float, help="Latitude for nearby queries")
    ...
    parser.add_argument("--south", type=float, help="Southern latitude for box queries")
    ...
    parser.add_argument(
        "--limit", type=int, default=100000, help="Maximum number of results"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    try:
        ...
        if args.action == "stats":
            ...
            if None in [args.west, args.south, args.east, args.north]:
                logger.error(
                    "Missing bounding box parameters (west, south, east, north)"
                )
                return 1
            ...
            # Query nearby points
            results = query.find_nearby(args.lat, args.lng, args.distance, args.limit)

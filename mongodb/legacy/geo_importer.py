    ...
    def import_proposal_locations(self, file_path: str) -> int:
        ...
        try:
            ...
            if documents:
                ...
                logger.info(
                    f"Inserted {len(result.inserted_ids)} proposal location documents"
                )
    ...
    def import_ess_dive_packages(self, file_path: str) -> int:
        ...
        try:
            ...
            if documents:
                ...
                result = self.collection.insert_many(documents)
                logger.info(
                    f"Inserted {len(result.inserted_ids)} ESS-DIVE package documents"
                )
                return len(result.inserted_ids)
            else:
    ...
    def import_nmdc_biosamples(self, file_path: str) -> int:
        ...
        try:
                    ...
                    documents.append(
                        {
                            ...
                            "metadata": {"source": "NMDC-Biosample"},
                        }
                    )
                except (ValueError, TypeError) as e:
            ...
            if documents:
                ...
                result = self.collection.insert_many(documents)
                logger.info(
                    f"Inserted {len(result.inserted_ids)} NMDC biosample documents"
                )
                return len(result.inserted_ids)
            else:
    ...
    def import_jgi_gold_biosamples(self, file_path: str) -> int:
        ...
        try:
                    ...
                    documents.append(
                        {
                            ...
                            "metadata": {"source": "JGI-GOLD-Biosample"},
                        }
                    )
                except (ValueError, TypeError) as e:
            ...
            if documents:
                ...
                result = self.collection.insert_many(documents)
                logger.info(
                    f"Inserted {len(result.inserted_ids)} JGI GOLD biosample documents"
                )
                return len(result.inserted_ids)
            else:
    ...
    def import_jgi_gold_organisms(self, file_path: str) -> int:
        ...
        try:
                    ...
                    documents.append(
                        {
                            ...
                            "metadata": {"source": "JGI-GOLD-Organism"},
                        }
                    )
                except (ValueError, TypeError) as e:
            ...
            if documents:
                ...
                result = self.collection.insert_many(documents)
                logger.info(
                    f"Inserted {len(result.inserted_ids)} JGI GOLD organism documents"
                )
                return len(result.inserted_ids)
            else:
...
def main():
    ...
    if not args.skip_large_files:
        if files_valid[3]:
            try:
                logger.info(
                    "Importing JGI GOLD biosamples (large file, this may take a while)..."
                )
        ...
        if files_valid[4]:
            try:
                logger.info(
                    "Importing JGI GOLD organisms (large file, this may take a while)..."
                )
